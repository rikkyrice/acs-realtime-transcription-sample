...
from azureAISpeech import AISpeech
...
def session_config():
    ...
    SESSION_CONFIG={
        ...
        "instructions": "あなたの名前はサムです。あなたはContoso Servicesで働いでいます。あなたは穏やかで陽気なエージェントで、丁寧な日本語で応答します。常に元気な声で返事をしてください。",
...
class OpenAIRTHandler():
    ...
    def __init__(self) -> None:
        ...
        self.connection_manager = self.client.beta.realtime.connect(
                    model="gpt-realtime-mini"  # Replace with your deployed realtime model id on Azure OpenAI.
            )
...
#start_conversation > start_client
    async def start_client(self):
            ...
            await self.ai_speech.start_recognition()
                ...
                async for event in self.connection:
                    ...
                    match event.type:
                        ...
                        case "conversation.item.input_audio_transcription.completed":
                            print(f" User (whisper):-- {event.transcript}")
                            pass
                        ...
                        case "response.audio_transcript.done":
                            print(f" AI (whisper):-- {event.transcript}")
                            pass
                        case "response.audio.delta":
                            await self.oai_to_acs(event.delta)
                            await self.ai_speech.send_audio_str(audio_data=event.delta)
                            pass
                        case _:
                            pass
    ...
    async def send_welcome(self):
        if not self.welcomed:
            ...
            item={
                "type": "message",
                "role": "user",
                "content": [{"type": "input_text", "text": "こんにちは！あなたの名前と職業を教えてください。"}],
                }
            )
            await self.connection.response.create()
            self.welcomed = True
